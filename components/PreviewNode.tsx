...
export const PreviewNode: React.FC<PreviewNodeProps> = memo(({ id, data, selected }) => {
  ...
  return (
    <div className={`relative w-32 h-32 rounded-lg overflow-hidden bg-zinc-950 border shadow-2xl transition-all group ${selected ? 'border-blue-500 ring-1 ring-blue-500/20' : 'border-zinc-800'}`}>
      ...
      {scalarValue && (
        <div className="absolute bottom-1 right-1 max-w-[110px] px-1.5 py-0.5 bg-zinc-950/80 backdrop-blur-sm rounded-sm border border-white/10 pointer-events-none flex items-center justify-end z-10 shadow-lg">
            <span className="font-mono text-[8px] text-zinc-300 truncate">{scalarValue}</span>
        </div>
      )}
      ...
      <div className="absolute top-1/2 -left-3 transform -translate-y-1/2 z-20">
         <Handle 
            ...
            className="!w-2.5 !h-2.5 !relative !left-0 !transform-none border-2 border-zinc-950 transition-transform hover:scale-125 shadow-lg" 
         />
